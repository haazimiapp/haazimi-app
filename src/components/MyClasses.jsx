...
const T = {
  en: {
    ...
    studentNameLabel: 'Student Name *', phoneLabelOpt: 'Phone (optional)',
    ...
    intentions: 'Intentions', complete: '✓ Complete',
    attendance: 'Attendance %', status: 'Status', notes: 'Notes',
    eightIntentions: '8 Intentions', tapToggle: '— tap each to toggle',
    confirmRemove: 'Remove this student from the class?',
    editStudent: 'Edit student', removeStudent: 'Remove student',
    quran: "Qur'aan", surahs: 'Surahs', duas: "Du'aas", muzaakarahs: 'Muzaakarahs', other: 'Other',
  ...
  ar: {
    ...
    studentNameLabel: 'اسم الطالب *', phoneLabelOpt: 'الهاتف (اختياري)',
    noStudentsInClass: 'لا يوجد طلاب في هذا الفصل.',
    cancel: 'إلغاء', save: 'حفظ', add: 'إضافة',
    studentCount: (n) => `${n} طالب`,
    syncing: '⟳ مزامنة…', synced: '✓ تمت',
    intentions: 'النيات', complete: '✓ مكتمل',
    attendance: 'الحضور %', status: 'الحالة', notes: 'ملاحظات',
    eightIntentions: '8 نيات', tapToggle: '— اضغط للتبديل',
    confirmRemove: 'هل تريد حذف هذا الطالب من الفصل؟',
    editStudent: 'تعديل الطالب', removeStudent: 'حذف الطالب',
    quran: "القرآن", surahs: 'السور', duas: "الأدعية", muzaakarahs: 'المذاكرات', other: 'أخرى',
  ...
  ur: {
    ...
    studentNameLabel: 'طالب کا نام *', phoneLabelOpt: 'فون (اختیاری)',
    ...
    intentions: 'نیتیں', complete: '✓ مکمل',
    attendance: 'حاضری %', status: 'حالت', notes: 'نوٹس',
    eightIntentions: '8 نیتیں', tapToggle: '— ٹوگل کریں',
    confirmRemove: 'کیا آپ اس طالب کو کلاس سے ہٹانا چاہتے ہیں؟',
    editStudent: 'طالب میں ترمیم', removeStudent: 'طالب ہٹائیں',
    quran: "قرآن", surahs: 'سورتیں', duas: "دعائیں", muzaakarahs: 'مذاکرات', other: 'دیگر',
  ...
  es: {
    ...
    studentNameLabel: 'Nombre del Estudiante *', phoneLabelOpt: 'Teléfono (opcional)',
    ...
    intentions: 'Intenciones', complete: '✓ Completo',
    attendance: 'Asistencia %', status: 'Estado', notes: 'Notas',
    eightIntentions: '8 Intenciones', tapToggle: '— toca para alternar',
    confirmRemove: '¿Eliminar este estudiante de la clase?',
    editStudent: 'Editar estudiante', removeStudent: 'Eliminar estudiante',
    quran: "Qur'aan", surahs: 'Suras', duas: "Du'aas", muzaakarahs: 'Muzaakarahs', other: 'Otro',
  ...
  pt: {
    ...
    studentNameLabel: 'Nome do Aluno *', phoneLabelOpt: 'Telefone (opcional)',
    noStudentsInClass: 'Nenhum aluno nesta turma ainda.',
    cancel: 'Cancelar', save: 'Salvar', add: 'Adicionar',
    studentCount: (n) => `${n} aluno${n !== 1 ? 's' : ''}`,
    syncing: '⟳ Sincronizando…', synced: '✓ Sincronizado',
    intentions: 'Intenções', complete: '✓ Completo',
    attendance: 'Frequência %', status: 'Estado', notes: 'Notas',
    eightIntentions: '8 Intenções', tapToggle: '— toque para alternar',
    confirmRemove: 'Remover este aluno da turma?',
    editStudent: 'Editar aluno', removeStudent: 'Remover aluno',
    quran: "Qur'aan", surahs: 'Suras', duas: "Du'aas", muzaakarahs: 'Muzaakarahs', other: 'Outro',
  ...
  return (
    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
      {Array.from({ length: 8 }, (_, i) => {
        const done = !!(mask & (1 << i));
        return (
          <button key={i} type="button" title={`Intention ${i + 1}`}
...
function StudentCard({ student, onSave, onDelete, t }) {
  ...
  const selStyle = { padding: '5px 8px', borderRadius: 6, border: '1px solid var(--border-color)', background: 'var(--input-bg)', color: 'var(--text-primary)', fontSize: '0.83rem', width: '100%', boxSizing: 'border-box' };
    ...
    <div className={`student-card ${editing ? 'student-card--editing' : ''}`}>
      <div className="student-card-header" onClick={() => !editing && setExpanded(e => !e)}>
        <div className="student-card-name-row">
          ...
          <span className={`status-badge ${STATUS_CLASS[student.status] || 'status-in-progress'}`}>{(t.statusLabels && t.statusLabels[student.status]) || student.status}</span>
        </div>
        <div className="student-card-actions" onClick={e => e.stopPropagation()}>
          {!editing && (
            <>
              <button className="icon-btn" onClick={startEdit} title={t.editStudent}><Edit2 size={15} /></button>
              <button className="icon-btn icon-btn--danger" onClick={() => onDelete(student.id)} title={t.removeStudent}><Trash2 size={15} /></button>
      ...
      <div style={{ marginTop: 6, fontSize: '0.78rem', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 6 }}>
        {t.intentions}: <strong style={{ color: doneCount === 8 ? '#6366f1' : 'var(--text-primary)' }}>{doneCount}/8</strong>
        {doneCount === 8 && <span style={{ color: '#6366f1', fontSize: '0.72rem' }}>{t.complete}</span>}
      </div>
        ...
        <div className="student-card-body">
          {editing ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <ProgressRow label={t.muzaakarahs} value={draft.muzaakarahs} editing onChange={v => d('muzaakarahs', v)} placeholder="e.g. Salaah, Wudhu, Kalimah" />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingTop: 8 }}>
              ...
              <ProgressRow label={t.muzaakarahs} value={student.muzaakarahs} />
...
export default function MyClasses({ user, language }) {
  ...
  const handleAdd = async () => {
    ...
    await addStudent({ name: newName.trim(), phone: newPhone.trim(), className: activeClass, attendance: 90, status: 'On Track', intentions: 0, quranJuz: '', surahs: '', duas: '', muzaakarahs: '', other: '', notes: '' });
    setNewName(''); setNewPhone(''); setShowAddStudent(false);
  };
              ...
              <div>
                <div className="class-detail-header">
                  <div className="class-name-row">
                    <h3>{activeClass}</h3>
                    <span style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginLeft: 8 }}>{t.studentCount(n)}</span>
                  </div>
                ...
                {showAddStudent && (
                  <div className="add-student-form" style={{ marginBottom: 16 }}>
                    <h4>{t.addTo(activeClass)}</h4>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 10 }}>
                      <div className="form-group student-edit-full" style={{ margin: 0 }}>
                      ...
                      <div className="form-group student-edit-full" style={{ margin: 0 }}>
                        <label>{t.phoneLabelOpt}</label>
                        <input type="tel" value={newPhone} onChange={e => setNewPhone(e.target.value)} placeholder="+27 82 000 0000" />
                      </div>
                    </div>
                    <div className="student-edit-actions">
                      <button type="button" className="button-secondary" onClick={() => setShowAddStudent(false)}><X size={14} /> {t.cancel}</button>
                      <button type="button" className="button-primary" onClick={handleAdd} disabled={!newName.trim()}><Plus size={14} /> {t.add}</button>
                    </div>
                  </div>
                )}
                ...
                <div className="student-cards-list">
                  {classStudents.length === 0 ? (
                    ...
                    <StudentCard key={student.id} student={student} onSave={handleSave} onDelete={handleDelete} t={t} />
                  ))}
                </div>
              </div>
            )}
