...
export default function PeopleToVisit({ user, language }) {
  ...
  const [people, setPeople] = useState(() => {
    try {
      const saved = localStorage.getItem('haazimi_mulaaqaats');
  ...
  const logVisit = (id) => setPeople(ps => {
    const updated = ps.map(p => p.id === id ? { ...p, status: 'upcoming', lastVisit: new Date().toISOString().split('T')[0] } : p);
    localStorage.setItem('haazimi_mulaaqaats', JSON.stringify(updated));
    return updated;
  });
