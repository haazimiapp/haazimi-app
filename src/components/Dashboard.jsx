...
function QuickLeaveRequest({ userName, userKey, staffNames, t, language }) {
  ...
  const doSubmit = async () => {
    ...
    // POST to Google Sheets so the leave is findable when a manager approves/rejects it
    try {
      const { GOOGLE_SCRIPT_URL: GS_URL } = await import('../data/config');
      await fetch(GS_URL, {
        method: 'POST',
        mode: 'no-cors',
        ...
        body: JSON.stringify({
          ...
          teacherName: entry.requestFor,
          teacherEmail: userKey || '',
...
function ManagerDashboard({ user, onNavigate, language }) {
  ...
  const setToggle = async (centre, feature, val) => {
    ...
    // 3. Sync to Google Sheets so OTHER users see the change
    try {
