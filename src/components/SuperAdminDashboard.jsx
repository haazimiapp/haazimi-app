...
function InsightsTab({ t }) {
  ...
  const teachingLogs = logs.filter(l => ['Teaching', 'Teaching + Admin', 'Teaching + Planning'].includes(l.activity));
...
export default function SuperAdminDashboard({ user, language, onNavigate }) {
  ...
  const statCards = [
    { label: t.totalUsers, value: loading ? '…' : accounts.length, color: '#6366f1' },
    { label: t.pendingApprovals, value: loading ? '…' : pendingUsers.length, color: '#f59e0b' },
    { label: t.approved, value: loading ? '…' : approvedUsers.length, color: '#10b981' },
    { label: t.countries, value: loading ? '…' : Object.keys(byCountry).length, color: '#8b5cf6' },
  ];
  ...
  return (
    <div>
      ...
      {activeTab === 'insights' && <InsightsTab t={t} />}
    </div>
  );
}
